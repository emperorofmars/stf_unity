...
{
	[AddComponentMenu("STF/Resources/stfexp/stfexp.collider.capsule")]
	[HelpURL("https://docs.stfform.at/resources/stfexp/stfexp_collider_capsule.html")]
	...
	public class STFEXP_Collider_Capsule_Handler : ISTF_Handler
	{
		...
		{
			var r = Resource as STFEXP_Collider_Capsule;
			if (JsonResource.ContainsKey("radius")) r.radius = JsonResource.Value<float>("radius");
			if (JsonResource.ContainsKey("height")) r.height = JsonResource.Value<float>("height");

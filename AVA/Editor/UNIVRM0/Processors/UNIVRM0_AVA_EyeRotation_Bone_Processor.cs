...
{
	public class UNIVRM0_AVA_EyeRotation_Bone_Processor : ISTF_Processor
	{
		...
		public List<UnityEngine.Object> Process(ProcessorContextBase Context, ISTF_Resource STFResource)
		{
			...
			if (!animator) Context.Report(new STFReport("No Animator Component created!", ErrorSeverity.FATAL_ERROR, AVA_FaceMesh._STF_Type));

			var eyeRotation = STFResource as AVA_EyeRotation_Bone;

			(Context as AVAContext).AddTask("eye.rotation", STFResource.STF_Id, new Task(() => {
	...
	public class Register_UNIVRM0_AVA_EyeRotation_Bone_Processor
	{
		...
		{
			STF_Processor_Registry.RegisterProcessor(DetectorUNIVRM0.STF_UNIVRM0_AVATAR_CONTEXT, new UNIVRM0_AVA_EyeRotation_Bone_Processor());
		}
	}
}

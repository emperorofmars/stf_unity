...
{
	public class STFEXP_Collider_Serializer : IUnity_Serializer
	{
		...
		public List<SerializerResult> Serialize(UnitySerializerContext Context, Object UnityObject)
		{
			...
			{
				var ret = new JObject() {
					{ "type", STFEXP_Collider_Capsule._STF_Type},
					{ "radius", collider.radius },
				...
				return new List<SerializerResult>{new() {
					STFType = STFEXP_Collider_Capsule._STF_Type,
					Origin = UnityObject,
					Result = ret,
					IsComplete = true,
					Confidence = SerializerResultConfidenceLevel.MANUAL,
			...
			{
				var ret = new JObject() {
					{ "type", STFEXP_Collider_Plane._STF_Type},
					{ "offset_position", TRSUtil.SerializeVector3(collider.position) },
					{ "offset_rotation", TRSUtil.SerializeQuat(collider.rotation) },
				};
				return new List<SerializerResult>{new() {
					STFType = STFEXP_Collider_Plane._STF_Type,
					Origin = UnityObject,
					Result = ret,
					IsComplete = true,
					Confidence = SerializerResultConfidenceLevel.MANUAL,
	...
	public class Register_VRC_Collider_Serializer
	{
		...
		{
			Unity_Serializer_Registry.RegisterSerializer(new STFEXP_Collider_Serializer());
		}
	}
}
